...
(defrule abrir_plantilla_sucesiones_logicas
    (declare (salience 20))
    =>
    (open "./tfg_web/reglas/plantilla_sucesiones_logicas" mydata_sucesiones_logicas)
...
(defrule mostrar_segundas_inv_consecutivas
    ...
    (printout data "En la " ?parte "º parte del compás "?compas " hay dos acordes en segunda inversión consecutivos.." )
    (close data)
)
...
(defrule mostrar_sucesion_erronea_1
    ...
    (bind ?parte (+ (div (mod ?i ?a) ?p) 1))
    (open "./tfg_web/fallos/fallos_mod3" data "a")
    (printout data "En la " ?parte "º parte del compás "?compas " se da la sucesión de acordes I-II en estado fundamental. El enlace de estos dos acordes en estado fundamental genera movimientos de quintas u octavas, además de dificultar la conducción de las voces para crear un buen contrapunto.." )
    (close data)
)
...
(defrule mostrar_sucesion_erronea_2
    ...
    (bind ?parte (+ (div (mod ?i ?a) ?p) 1))
    (open "./tfg_web/fallos/fallos_mod3" data "a")
    (printout data "En la " ?parte "º parte del compás "?compas " se da la sucesión de acordes II-III en estado fundamental, la cual es incorrecta. El enlace de estos dos acordes en estado fundamental genera movimientos de quintas u octavas, además de dificultar la conducción de las voces para crear un buen contrapunto. También añadir que está sucesión de acordes da una sonoridad algo extraña dado que la función de subdominante del II grado tiende a moverse hacia una dominante y no al III grado.." )
    (close data)
)
...
(defrule mostrar_sucesion_erronea_3
    ...
    (bind ?parte (+ (div (mod ?i ?a) ?p) 1))
    (open "./tfg_web/fallos/fallos_mod3" data "a")
    (printout data "En la " ?parte "º parte del compás "?compas " se da la sucesión de acordes III-IV en estado fundamental, la cual es incorrecta. El enlace de estos dos acordes en estado fundamental genera movimientos de quintas u octavas, además de dificultar la conducción de las voces para crear un buen contrapunto.También añadir que está sucesión de acordes da una sonoridad algo extraña dado que el III grado tiende a moverse hacia el VI grado y no hacia una subdominante..")
    (close data)
)
...
(defrule mostrar_acordes_incompletos
    ...
    (bind ?parte (+ (div (mod ?i ?a) ?p) 1))
    (open "./tfg_web/fallos/fallos_mod3" data "a")
    (printout data "En la " ?parte "º parte del compás "?compas " hay un acorde incompleto. Los acordes no pueden estar incompletos bajo ningún concepto, ya que provocan una pérdida del soporte armónico y una sonoridad pobre.." )
    (close data)
)